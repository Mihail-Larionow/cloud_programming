...
Система предупреждила о том, что посещение данного сайта может быть небезопасно. Так происходит, потому что был использован самоподписанный сертификат, а по умолчанию Mozilla Firefox и многие другие современные браущ=зеры не доверяют таким сертификатам.
...
Таким образом, были получены базовые навыки работы с платформой для автоматического развертывания, масштабирования и управления контейнеризованными приложениями Kibernetes. Были использованы разные объекты его API для выполнения работы – Service, Deployment, Ingress, Namespace. Было выполнено развертывание простого сервиса в кластере Kibernetes с использованием Docker в качестве драйвера на локальной машине. Было также выполнено развертывание стороннего сервиса httpd с самоподписанным сертификатом на хосте `minikube.webserver.ru`.
